...
const createNewOrder = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const  payload = req.body;
            ...
            if (!address || typeof address !== 'string' || !items || !Array.isArray(items) || items.length === 0) {
                console.log("hit here in");  
            ...
            const placeOrder = await orderService.createNewOrder(userId, address, items );
            if (!placeOrder) {
